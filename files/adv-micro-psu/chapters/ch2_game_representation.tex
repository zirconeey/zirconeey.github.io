...
Before introducing the normal form, we need to formalize what a \emph{strategy} is in an extensive-form game. The notion is rich: a strategy specifies what a player plans to do not just on the equilibrium path, but at every information set she might encounter.
...
\ex[]{
  ...
  In this game, Player 1's strategies are $LA$, $LB$, $RA$, and $RB$, where $LA$ means that Player 1 chooses $L$ at the first information set belonging to her, and chooses $A$ at the second. The strategies at all information sets will lead to a unique outcome.
}
...
\rmk{
  Pure strategy is a special case of mixed strategy, assigning probability one to a specific action.
}
...
\section{Dominated Strategies and Best Responses}

Rather than asking what is an optimal strategy, we approach it in reverse: what are the suboptimal strategies that a rational player would never choose, so that we can rule them out in the analysis of equilibrium?
...
The previous theorem states that dominated strategy is a belief-free notion. This allows us to develop the concept of iterated elimination of dominated strategies (IESDS), which is a powerful tool to analyze games without having to worry about players' beliefs.
...
\end{center}
\begin{itemize}
  \item If Player 1 eliminates $U$ and $D$, then Player 2 can eliminate nothing, and the process ends. The surviving strategies are $M$ for Player 1, and $L$ and $R$ for Player 2.
  \item If Player 1 eliminates only $U$, then Player 2 can eliminate $L$, and the process ends. The surviving strategies are $M$ and $D$ for Player 1, and $R$ for Player 2.
  \item If Player 1 eliminates only $D$, then Player 2 can eliminate $R$, and the process ends. The surviving strategies are $U$ and $M$ for Player 1, and $L$ for Player 2.
\end{itemize}
From this example, we can clearly see the ``outcome'' of IEWDS depends on the order of elimination.
}

\rmkb[Chapter Summary]{
  This chapter introduced two complementary representations of a game and the language we will use throughout the book. The \emph{extensive form} (game tree, with information sets capturing what each player knows when she moves) is the natural object for sequential-move games and games of imperfect information. The \emph{normal form} (a tuple of strategy sets and payoff functions) is the natural object for simultaneous-move analysis and the home of equilibrium concepts; every extensive-form game admits a normal-form representation by enumerating strategies. A \emph{strategy} sits between the two: it is defined on the extensive form (a complete plan for every information set) but its payoff implications live in the normal form. The chapter closes with the first nontrivial solution-concept idea---\emph{dominated strategies and iterated elimination}---which already shows that rationality alone, without any equilibrium fixed-point reasoning, can pin down behavior in some games. Chapter~3 introduces the central equilibrium concept, Nash equilibrium, that the rest of the book builds on.
}
