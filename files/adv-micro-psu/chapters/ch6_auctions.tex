...
\defn{Private-Value Auction Setting}{
  A \textbf{private-value auction} consists of $n$ risk-neutral bidders competing for a single indivisible object. Each bidder $i$ independently draws a valuation $X_i \sim F$, where $F$ is a continuous cumulative distribution function on $[0, \bar x]$ that is common knowledge. The realization $x_i$ is bidder $i$'s \emph{private information}---no other bidder, nor the seller, observes it. Each bidder submits a bid $b_i \in \RR_{\ge 0}$, and the auctioneer determines the winner and the payment from the bid profile $(b_1, \ldots, b_n)$ according to the auction's rules.
}

\section{Second Price Sealed Bid Auction (SPA)}

\defn{Second-Price Sealed-Bid Auction}{
  Each bidder simultaneously submits a sealed bid. The bidder with the highest bid wins the object and pays the \emph{second}-highest bid; ties are broken uniformly at random.
}
...
\end{cases}
\]
This corresponds to the \textbf{black} line in the figure below.
...
\end{cases}
\]
This corresponds to the {\color{red}\textbf{red}} line in the figure below.
...
\end{cases}
\]
This corresponds to the {\color{blue}\textbf{blue}} line in the figure below.
...
\defn{First-Price Sealed-Bid Auction}{
  Each bidder simultaneously submits a sealed bid. The highest bidder wins and pays \emph{her own} bid; ties are broken uniformly at random.
}
...
\defn{Reserve Price}{
  A \textbf{reserve price} $r \ge 0$ is a minimum acceptable bid: the auction sells the object only if at least one submitted bid satisfies $b_i \ge r$. If all bids fall below $r$, the seller retains the object and no payment is made.
}
